        ...
        <div className="fixed inset-0 z-50 bg-slate-950/20" onMouseDown={(event) => event.target === event.currentTarget && setForm(null)}>
          <aside role="dialog" aria-modal="true" aria-labelledby="profile-editor-title" className="ml-auto flex h-full w-[560px] flex-col border-l border-slate-200 bg-white shadow-2xl">
            ...
            <div className="flex-1 space-y-5 overflow-y-auto p-6">
              ...
              {form.site_key ? <div className="rounded-lg bg-slate-50 p-4 text-sm"><div className="font-medium">站点能力</div><p className="mt-2 text-slate-600">注册 · 登录 · 密钥管理{sites.find((site) => site.key === form.site_key)?.checkin_supported ? " · 签到" : ""}</p><p className="mt-2 text-xs leading-5 text-slate-500">邮箱域名白名单由已验证站点目录统一下发，不在站点中手工配置。</p></div> : null}
            </div>
